...
export function LocalStorageTab({ searchTerm, onSearchChange }: LocalStorageTabProps) {
  ...
  const handleEdit = (item: LocalStorageItem) => {
  ...
  const handleSaveEdit = () => {
    ...
    try {
      setLocalStorageItem(editingKey, editingValue);
      setEditingKey(null);
      setEditingValue('');
      loadItems();
    } catch (err) {
    ...
    <div className="space-y-4">
      ...
      {filteredItems.length === 0 ? (
        <div className="text-center py-8 text-gray-500 dark:text-gray-400">
          {searchTerm ? `No items found matching "${searchTerm}"` : 'No items in Local Storage'}
        </div>
      ) : (
