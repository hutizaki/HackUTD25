...
import { FeatureFlagList } from './Admin/FeatureFlagList';
...
export function FeatureFlagsPanel({ isOpen, onClose, onMinimizedChange }: FeatureFlagsPanelProps) {
  ...
  useEffect(() => {
    localStorage.setItem('featureFlagsPanelState', JSON.stringify(panelState));
  }, [panelState]);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="bg-purple-600 text-white px-4 py-2 cursor-move flex items-center justify-between select-none"
      >
        <div className="flex items-center space-x-2">
          <h2 className="text-lg font-semibold">Feature Flags</h2>
          {panelState.minimized && (
            <span className="text-xs text-purple-200">(Minimized)</span>
          )}
        </div>
        <div className="flex items-center space-x-2">
          <button
